...
class LivestreamShareSheet extends StatefulWidget {
  final int livestreamId;
  final String title;
  final String coverUrl;
  final String anchorName;
  final int initialTab; // 0=好友, 1=群聊

  const LivestreamShareSheet({
    super.key,
  ...
  @override
  State<LivestreamShareSheet> createState() => _LivestreamShareSheetState();
}

class _LivestreamShareSheetState extends State<LivestreamShareSheet>
  ...
  Future<void> _sendShare() async {
    ...
    final extra = jsonEncode({
      ...
      'anchor_name': widget.anchorName,
    });

    // 使用标题作为消息内容（服务端要求content非空）
    final content = widget.title.isNotEmpty
        ? widget.title
        : (l10n?.shareLivestream ?? 'Share Livestream');
    ...
    try {
      ...
      for (final userId in _selectedUserIds) {
        await _conversationApi.sendMessage(
          msgId: const Uuid().v4(),
          toUserId: userId,
          type: MessageType.livestreamShare,
          content: content,
          extra: extra,
        );
      }
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text(l10n?.translate('sv_share_success') ?? 'Shared successfully')),
        );
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('${l10n?.shareFailed ?? 'Share failed'}: $e')),
        );
      }
    } finally {
          ...
          child: Column(
            children: [
              ...
              TabBar(
                ...
                tabs: [
                  Tab(text: l10n?.translate('contacts') ?? 'Contacts'),
                  Tab(text: l10n?.groupChat ?? 'Group Chat'),
                ],
              ),
                  ...
                  child: SizedBox(
                    ...
                    child: ElevatedButton(
                      ...
                      child: _isSending
                          ...
                          : Text(
                              selectedCount > 0
                                  ? '${l10n?.translate("send") ?? "Send"}($selectedCount)'
                                  : l10n?.translate('send') ?? 'Send',
                              style: const TextStyle(
                                fontSize: 16,
                                color: Colors.white,
                              ),
  ...
  Widget _buildFriendList(AppLocalizations? l10n) {
    if (_friends.isEmpty) {
      return Center(
        child: Text(
          l10n?.translate('no_friends') ?? 'No friends yet',
  ...
  Widget _buildGroupList(AppLocalizations? l10n) {
    if (_groups.isEmpty) {
      return Center(
        child: Text(
          l10n?.translate('no_groups') ?? 'No groups yet',
          style: TextStyle(color: AppColors.textSecondary),
        ),
      );
    }
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        return ListTile(
          ...
          subtitle: Text(
            (l10n?.translate('people_count') ?? '{count}')
